...
class EEGFeatNet(nn.Module):
    def __init__(self, in_channels, n_features, projection_dim, num_layers):
        ...
        self.encoder    = nn.LSTM(input_size=in_channels, hidden_size=n_features, num_layers=self.num_layers, batch_first=True)
...
if __name__ == '__main__':
    ...
    model = EEGFeatNet(in_channels=config.input_size ,n_features=config.feat_dim, projection_dim=config.projection_dim,num_layers=config.num_layers).to(config.device)
    proj, feat = model(eeg)
